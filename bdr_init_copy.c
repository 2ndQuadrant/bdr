...
static void node_join(PGconn *conn, char *remote_connstr, char *local_connstr);
...
main(int argc, char **argv)
{
	...
	for (i = 0; i < remote_info->numdbs; i++)
	{
		...
		node_join(local_conn, remote_connstr, local_connstr);
...
static void
node_join(PGconn *conn, char *remote_connstr, char *local_connstr)
{
	PQExpBuffer  query = createPQExpBuffer();
	PGresult	*res;
	...
	printfPQExpBuffer(query, "SELECT bdr.node_join(%s, %s, %s);",

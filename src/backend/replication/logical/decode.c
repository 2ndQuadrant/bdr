		...
		case XLOG_RUNNING_XACTS:
			{
				 ...
				 * than the record's oldestRunningXid. This is the most
				 * convenient spot for doing so since, in contrast to shutdown
				 * or end-of-recovery checkpoints, we have sufficient
				 * knowledge to deal with prepared transactions here.
				 */
...
DecodeHeapOp(LogicalDecodingContext *ctx, XLogRecordBuffer *buf)
{
	...
	switch (info)
	{
		...
		case XLOG_HEAP_INPLACE:
			 ...
			 * record's contents.
			 *
			 * There are cases in which inplace updates are used without xids
			 * assigned (like VACUUM), there are others (like CREATE INDEX
			 * CONCURRENTLY) where an xid is present. If an xid was assigned
			 * the commit record will include cache invalidations, so we mark
			 * the transaction as catalog modifying here. Currently that's
			 * redundant because the commit will do that as well, but one
			 * we'll support decoding in-progress relations, this will be
			 * important.
			 */

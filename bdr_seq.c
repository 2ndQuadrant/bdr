...
"WHERE\n"
"    election.open\n"
"    -- only allow to vote for one node at a time. That allows us to do\n"
"    -- multiple votes in one batch, because one node won't ask for\n"
"    -- overlapping values.\n"
"    -- To do so, find a single source node with at least one open election\n"
"    -- that needs our vote.\n"
"    AND (owning_sysid, owning_tlid, owning_dboid, owning_riname) = (\n"
"        SELECT owning_sysid, owning_tlid, owning_dboid, owning_riname\n"
"        FROM bdr.bdr_sequence_elections open_electorate\n"
"        WHERE open_electorate.open\n"
"            -- not our election\n"
"            AND NOT (\n"
"                open_electorate.owning_sysid = $1\n"
"                AND open_electorate.owning_tlid = $2\n"
"                AND open_electorate.owning_dboid = $3\n"
"                AND open_electorate.owning_riname = $4\n"
"                )\n"
"            -- already voted\n"
"            AND NOT EXISTS (\n"
"                SELECT *\n"
"                FROM bdr_votes v\n"
"                WHERE true\n"
"                    AND open_electorate.owning_sysid = v.vote_sysid\n"
"                    AND open_electorate.owning_tlid = v.vote_tlid\n"
"                    AND open_electorate.owning_dboid = v.vote_dboid\n"
"                    AND open_electorate.owning_riname = v.vote_riname\n"
"                    AND open_electorate.owning_election_id = v.vote_election_id\n"
"\n"
"                    AND v.voter_sysid = $1\n"
"                    AND v.voter_tlid = $2\n"
"                    AND v.voter_dboid = $3\n"
"                    AND v.voter_riname = $4\n"
"            )\n"
"        \n"
"--        ORDER BY random()\n"
"        LIMIT 1\n"

...
session "s1"
step "s1_init" { SELECT 'init' FROM pg_create_decoding_replication_slot('isolation_slot', 'test_decoding'); }
step "s1_begin" { BEGIN; }
...
step "s2_get_changes" { SELECT regexp_replace(data, 'temp_\d+', 'temp') AS data FROM pg_decoding_slot_get_changes('isolation_slot', 'now', 'include-xids', '0'); }

...
bdr_perdb_worker(Datum main_arg)
{
	...
	if (bdr_needs_init())
	{
		char *connstr = get_conn_string();
		char status;
		min_remote_lsn;
		/* use PG_ENSURE_CLEANUP to drop connection to remote */
		PGconn *pqconn;
		...
		pgconn = do_connect_sql();
		PG_ENSURE_CLEANUP(.... pqconn .... )
		{
			/* See if we've started any setup work on the remote */
			(status, min_remote_lsn) = get_node_status_from_remote();

			/*
			 * Init could've been completed, but the final update to the
			 * bdr.bdr_nodes table might not have propagated yet.
			 */
			if (status == 'r')
				/*
				 * All we need to do is catch up, we already replayed enough
				 * to be consistent and start up in normal mode last time around
				 */
				return;
			...
			if (status == 'i')
			{
				 ...
				 */
				drop_slot_if_exists();
				drop_local_identifier_if_exists();
				drop_remote_status();
				status = '\0';
			}
			...
			if (status == '\0')
			{
				...
				set_remote_status('i');
				create_slots_on_all_nodes_if_not_exists(); /* or launch workers and wait for them using latch and cond var? */
				apply_dump();
				set_remote_status('c');
				min_remote_lsn = get_remote_lsn();
			}

			Assert(status == 'c');

			catchup_to_lsn(min_remote_lsn, connstr);
			set_remote_status('r');
		}
	}

...
typedef struct MsgbConnection
{
	...
	uint64 msgb_msgid_counter;		/* next message ID to allocate */
...
#define INITIAL_MAX_CONNS 10
	...
	conns = MemoryContextAlloc(msgbuf_context,
							   sizeof(MsgbConnection) * nconns);
	memset(conns, 0, sizeof(MsgbConnection) * nconns);
...
msgb_send_next(MsgbConnection *conn)
{
	...
	snprintf(msgid_str, 30, INT64_FORMAT, (int64) buf->msgid);
...
msgb_service_connections_events(WaitEvent *occurred_events, int nevents)
{
	...
	for (i = 0; i < nevents; i++)
	{
		...
		for (j = 0; j < nconns; j++)
		{
			if (conns[j].destination_id != 0 && conns[j].wait_set_index == e->pos)
			{
				conn = &conns[j];
				/* Sanity check only */
 ...
 */
void
msgb_wakeup_send(WaitEvent *occurred_events, int nevents,
 ...
 */
MsgbSendStatus
msgb_message_status(uint32 destination, uint64 msgid)
{
	ListCell *lc;
	MsgbMessageBuffer *msg = NULL;
	uint64 next_pending_msgid;
	int idx;
	MsgbConnection *conn;
...
static void
msgb_remove_destination_by_index(int index, bool request_waitevents)
{
	...
	/*TODO, remove connection slots after some threshold (<50% used?) */
}
...
msgb_add_send_peer(uint32 destination_id, const char *dsn)
{
	...
	/* If not found allocate some more slots (double the current amount) */
	if (conn == NULL)
	{
		uint32 oldnconns = nconns;
		nconns *= 2;

		conns = repalloc(conns, sizeof(MsgbConnection) * nconns);
		memset(conns + sizeof(MsgbConnection) * oldnconns, 0,
			   sizeof(MsgbConnection) * (nconns - oldnconns));

		for (i = oldnconns; i < nconns; i++)
		{
			conns[i].wait_set_index = -1;
			/* msgid 0 is reserved for setup */
			conns[i].msgb_msgid_counter = 1;
